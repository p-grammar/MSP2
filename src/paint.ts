...
function sizeCanvas(canvas: HTMLDivElement, parent: HTMLDivElement)
{
...
function fillCanvas(canvas: HTMLDivElement, width: number, height: number)
{
	while(canvas.hasChildNodes()) {
		canvas.removeChild(<ChildNode>canvas.lastChild);
	}

	for(let i = 0; i < width; ++i) {
		for(let j = 0; j < height; ++j) {
			createDiv(canvas, 'pixel').style.backgroundColor = 'white'; //(Math.random() > 0.5) ? 'white' : 'black';
		}
	}
}

async function massPaint(canvas: HTMLDivElement, x: number, y: number, s: number, color: {r:number, g:number, b:number})
{
...
async function subPaint(canvas: HTMLDivElement, x: number, y: number, color: {r:number, g:number, b:number})
{
	(<HTMLElement>canvas.children[y * image_width + x]).style.backgroundColor = `rgb(${color.r},${color.g},${color.b})`;
}
